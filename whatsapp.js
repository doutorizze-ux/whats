...
import { useMySQLAuthState, getAllSessionIds, removeAllSessionData } from './store/mysql-auth.js'
...
const createSession = async (sessionId, res = null, options = { usePairingCode: false, phoneNumber: '' }) => {
    ...
    const { state, saveCreds } = await useMySQLAuthState(dbConfig, sessionId)
...
const deleteSession = (sessionId) => {
    ...
    removeAllSessionData(dbConfig, sessionId)
...
const init = async () => {
    const sessionIds = await getAllSessionIds(dbConfig)
